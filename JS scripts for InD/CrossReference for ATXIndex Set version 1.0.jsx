...
// CrossReference for ATXIndex Set  version 1.0.jsx
//
// Modified 2019-03-14
// Igor Nikitin, Rls
//
...
function Main() {
  ...
  var doc1 = app.documents.itemByName('Glava_4.indd');
  ...
  try {
    if (!(doc0.name && doc1.name)) {
      throw new ReferenceError('Ошибка чтения данных');
    }
  } catch (e) {
    if (e.name == 'ReferenceError') {
      alert(
        'Для работы скрипта необходимо открыть документы Glava_1_5.indd, Glava_4.indd.indd. Откройте их и запустите скрипт снова'
      );
      return;
    } else {
      throw e;
  ...
  //установка номеров страниц перекрестных ссылок в АТХ Указателе (стиль абзацев и соотв. xml-элементов UkATC_Tname); поиск якорей для этих ссылок проводится в файле раздела главы 1 "Описания л.с., БАДов, мед. изделий"
  setSourcesForDocument(
    doc1,
    doc0,
    storeDrugDescriptionDestinationNames,
    'descendant::UkATC_TName/descendant::crossRef[@href]'
  );
